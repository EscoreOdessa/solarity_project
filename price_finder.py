...
# ══════════════════════════════════════════════════════════
# 3. ПОШУК ЦІН В ІНТЕРНЕТІ — ЧЕРЕЗ GOOGLE (як людина)
# ══════════════════════════════════════════════════════════
...
#     """Витягує ціну з поточної сторінки товару."""
...
#         except Exception:
#             pass
#     # Fallback: regex по тексту — беремо МІНІМАЛЬНУ ціну (не медіану)
#     try:
...
#     """Шукає в Google і повертає список URL результатів."""
#     google_url = f"https://www.google.com/search?q={quote_plus(query)}&hl=uk&gl=ua&num=10"
#     try:
...
#     """
#     Витягує ціни прямо зі сніпетів Google пошуку.
#     Google вже показує ціни у результатах — не треба заходити на сайти.
#     """
#     try:
...
#             // Перебираємо всі блоки результатів
#             const blocks = document.querySelectorAll(
#                 '[data-hveid], .g, .tF2Cxc, [jscontroller]'
#             );
...
#                         found.push({price: val, url: href});
#                         break; // одна ціна на блок
#                     }
#                 }
#             }
#             return found;
...
#     """
#     Шукає ціну через Google як людина — використовує власний Chromium зі stealth.
#     НЕ чіпає реальний браузер користувача.
#       Крок 1: Відкриваємо Google з запитом
#       Крок 2: Витягуємо ціни зі сніпетів Google (якщо є) — швидко
#       Крок 3: Якщо сніпети порожні — заходимо на кожен результат окремо
#     """
#     try:
...
#                 f"?q={quote_plus(google_query)}&hl=uk&gl=ua&num=10"
#             )

#             await page.goto(google_url, timeout=30_000, wait_until="domcontentloaded")
#             await asyncio.sleep(2.5)

#             # Крок 2: ціни зі сніпетів Google (без переходу на сайти)
#             snippet_prices = await _get_prices_from_google_snippets(page)
#             if snippet_prices:
#                 print(f"   ✅ Знайдено {len(snippet_prices)} цін прямо в Google:")
...
#                         print(f"   ⛔ Ігнорую {site}")
#                         continue
#                     found_prices.append(price)
#                     found_sources.append(f"{site}: {url}")
#                     print(f"   💰 {price:,.2f} грн — {site}")

#             # Крок 3: якщо сніпети дали мало цін — заходимо на сайти
...
#                 else:
#                     print(f"   Заходжу на {len(result_urls)} сайтів...")
#                     for i, url in enumerate(result_urls, 1):
#                         from urllib.parse import urlparse as _urlparse
#                         site_name = _urlparse(url).netloc.replace("www.", "")
#                         print(f"   [{i}] {site_name}: {url[:60]}")
#                         if any(skip in site_name for skip in SKIP_PRICE_SITES):
#                             print(f"   ⛔ Ігнорую {site_name}")
#                             continue
#                         try:
...
#                             if price:
#                                 found_prices.append(price)
#                                 found_sources.append(f"{site_name}: {page.url}")
#                                 print(f"   {Fore.GREEN}💰 {price:,.2f} грн{Style.RESET_ALL}")
#                             else:
...
#     # Відкидаємо мін і макс якщо є хоча б 3 значення
#     calc_prices = found_prices[:]
#     if len(calc_prices) >= 3:
#         calc_prices.remove(min(calc_prices))
#         calc_prices.remove(max(calc_prices))

#     avg = round(sum(calc_prices) / len(calc_prices), 2)
